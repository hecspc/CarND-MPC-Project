...
//    std::cout << "latency index: " << latency_index << std::endl;
  ...
  // creates a 2 element double vector.
    for (int i = 0; i < N - 1; i++) {
        path_x[i] = solution.x[x_start + i + 1];
        path_y[i] = solution.x[y_start + i + 1];
    }
    
//    steering_delta_ = a_ = 0.0;
//    for (int i = 0; i <= latency_index; i++)
//    {
//        steering_delta_ += solution.x[delta_start + i] / (latency_index + 1);
//        a_ += solution.x[a_start + i] / (latency_index + 1);
//    }
